        ...
        public static void Menu()
        {
            while (true)
            {
                int q = Menus.KeyboardMenu(
                    new[]
                    {
                    "5-1：將 10 個數字讀入陣列 A，逐一檢查：若 A[i] > 5，則 A[i] 減 5，否則加 5。",
                    "5-2：將10個數字讀入A陣列，對每一個數字，令A[i]=A[i]+i。",
                    "5-3：將 10 個數字讀入陣列 A，並建立陣列 B：若 A[i] >= 0，則 B[i] = 1，否則 B[i] = 0。",
                    "5-4：將15數字存入3×5的二維陣列A中，求每一行及每一列數字的和。",
                    "5-5：將15數字存入3×5的二維陣列A中，求每一行及每一列數字的最小值。",
                    "5-6：輸入兩組數字：a1,a2,…,a5和b1,b2,…,b5。求ai+bi，i=1到i=5。",
                    "5-7：輸入兩組數字 a1~a5 與 b1~b5，分別求其最大值 x、y，並輸出 x 與 y 中較小者。",
                    "回主選單"
                    },
                    "Array Practice（CH 5）"
                );
